...
class SnakeGameAI:
    ...
    def _is_iterating_too_longer(self):
        return self.frame_iterations > 100 * len(self.snake)
    ...
    # TODO: Update from `direction` to `action`
    def _move(self, action):
        # [straight, right, left]

        close_wise = [Direction.RIGHT, Direction.DOWN,
                      Direction.LEFT, Direction.UP]
        current_direction_index = close_wise.index(self.direction)

        if np.array_equal(action, [1, 0, 0]):  # straight, no change
            new_direction = close_wise[current_direction_index]
